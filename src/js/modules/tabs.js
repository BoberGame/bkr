function initTabs(tabsType = true) {
  ...
  const tabsNavigationDefault = (element, tabs) => {
    const btn = element.closest(TABS_NAV_ATTR);
    if (btn) {
      const navId = btn.dataset.tabsNav;
      return tabs[navId];
    }
  };
  ...
  function tabsHandler(event) {
    ...
    let tab = tabsNavigationDefault(target, tabs);
